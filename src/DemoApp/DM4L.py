...
time_real_version = f'{date} {real_time}' # 出社中デモ(会議直前)
...
class DecisionMaking():
    ...
    def WorkScheduleApp(self, Action_List, trigger, agent, model):
    ...
    def run(self):
        ...
        if not self.through:
            ...
            if self.transit_go_judge or self.transit_back_judge:
                ...
                # 予定管理アプリ
                ret = self.WorkScheduleApp(Action_List, trigger, agent, model) # modelはガイダンスのためだけに渡しているので、置き換える
