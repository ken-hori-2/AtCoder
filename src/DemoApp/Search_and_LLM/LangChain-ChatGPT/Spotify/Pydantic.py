...
"""
これが重要
action 文字列によって、 Spotify の楽曲再生 で呼び出すエンドポイントを決定しています。FieldのDescription にそれぞれの内容を説明させます。
"""

class MusicPlaybackAction(BaseModel):
    action: str = Field(description="The action to be performed. Supported actions are 'Play-Uptempo', 'Play-Relax', and 'Pause'.") # 他にもプレイリスト作成など
