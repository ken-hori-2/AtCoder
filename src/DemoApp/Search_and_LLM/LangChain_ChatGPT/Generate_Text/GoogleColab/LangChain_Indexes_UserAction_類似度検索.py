...
# 最初のプロンプト
# query = "Q1. 文書はユーザーの行動履歴です。このユーザーの一日の行動はどのような傾向がありますか？どのような時間や行動をするときにどんな機能を使う傾向にあるかをできるだけ、長く細かく教えて。"
query = "Q1. 文書はユーザーの行動履歴です。このユーザーの一日の行動はどのような傾向がありますか？どのような時間や行動をするときにどんな機能を使う傾向にあるかをできるだけ、長く細かく教えて。その際、使っている機能をすべて答え、どの時間でどの行動をしているときかも合わせて一日の時系列順にこたえて。(例:朝は～、午前中は～、昼は～、午後は～、夕方は～、夜は～)"
print(f"\n\n{query}")
# answer = index.query(query, llm = OpenAI(temperature=0))
# print(answer)
