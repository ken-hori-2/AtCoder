...
prompt = ChatPromptTemplate.from_messages([
    ...
    あなたが提案できる機能は
    ["予定管理", "楽曲再生", "経路検索", "ウェブ検索", "レストラン検索", "ニュース情報", "天気情報"]
    です。\
    また、日本語で回答しなければならない。\
    ### 出力形式は以下\n\
    提案する機能: "ここに機能名を出力"\
...
pre_Info = "朝方の出勤中や、夕方の帰宅中には電車や公共交通機関を使った移動をしたいので、経路検索を使う傾向が高いです。\
...
res = chain.invoke({"input": f"事前情報は{res}で、現在時刻は{time}です。ユーザーのニーズに合った機能を提案して。提案する機能を選定した理由も教えて。"})
print(res)
