...
# 2024/05/27 追加
# ユーザーの最終確認を追加（提案に対して、音声/ジェスチャーで実行するかどうかを決定するバージョン）
...
from Demo4Public.JudgeModel import Langchain4Judge
...
2024/05/28
置き換えたが、クラス化しているので、エラーになるかもしれない
"""
...
# # 現在時刻を手動で設定
dt_now = datetime.datetime(2024, 5, 27, 8, 58) # 通勤中
# dt_now = datetime.datetime(2024, 5, 27, 10, 55) # Mtg
...
dt_now = datetime.datetime(2024, 5, 27, 20, 00) # ジム

dt_now = datetime.datetime(2024, 5, 24, 8, 00) # テスト用
# print("現在時刻：", dt_now)
print("[DM4L_New.py] 現在時刻：", dt_now)
margin = 5
...
class DecisionMaking():
    ...
    def TravelByFootOrTrainApp(self, Action_List, trigger, agent, model): # Route guidance & music playback application
        ...
        if args in Action_List:
            if ("WALKING" in args) or ("RUNNING" in args): # 動いている場合
                ...
                print("##### 予定と行動状態から提案する機能: #####")
                print("*****\n経路案内\n*****") # 提案する機能
                # ここから追加 (2024/05/27)
                # model.text_to_speach("予定と行動状態から提案する機能は「経路案内」です。\n実行しますか？")
                userinterface.text_to_speach("予定と行動状態から提案する機能は「経路案内」です。\n実行しますか？") # 2024/05/28
                print("\n\n#####\nユーザーによる最終確認中...\n#####")
                print("***** センシング中 *****")
                # args = trigger.run()
                args = "RUNNING" # テスト用
                if ("WALKING" in args) or ("RUNNING" in args):
                    Final_Check = "YES"
                    # 音声入力の場合は入力の揺らぎをLLMによって最終出力をYES/NOにする
                    # ヘッドジェスチャーならYES/NO
                # Final_Check = "YES" # "NO" # "YES" # テスト用
                print("DNN検出結果：", Final_Check)
                print("***** センシング終了 *****")
                if Final_Check in "YES":
                    print("ユーザーの回答: ", Final_Check)
                    print(" > 実行します")
                    # model.text_to_speach("「経路案内」を実行します。")
                    userinterface.text_to_speach("「経路案内」を実行します。") # 2024/05/28
                ...
                print("*****\n楽曲再生\n*****") # 提案する機能
                # ここから追加 (2024/05/27)
                # model.text_to_speach("予定と行動状態から提案する機能は「楽曲再生」です。\n実行しますか？")
                userinterface.text_to_speach("予定と行動状態から提案する機能は「楽曲再生」です。\n実行しますか？") # 2024/05/28
                print("\n\n#####\nユーザーによる最終確認中...\n#####")
                print("***** センシング中 *****")
                # args = trigger.run()
                  # 音声入力の場合は入力の揺らぎをLLMによって最終出力をYES/NOにする
                  # ヘッドジェスチャーならYES/NO
                Final_Check = "YES" # テスト用
                print("DNN検出結果：", Final_Check)
                print("***** センシング終了 *****")
                if Final_Check in "YES":
                    print("ユーザーの回答: ", Final_Check)
                    print(" > 実行します")
                    # model.text_to_speach("「楽曲再生」を実行します。")
                    userinterface.text_to_speach("「楽曲再生」を実行します。") # 2024/05/28
        ...
        # 2024/05/28 一旦コメントアウト（繰り返されてしまう）
        ...
        #         userinterface.text_to_speach(final_response) # 2024/05/28
        #         # text_to_speach(response['output'])
        #     else:
        #         print("{{ または }} は含まれていないため、出力形式修正用のLLMは呼び出しません。")
        #         # model.text_to_speach(response['output'])
        #         userinterface.text_to_speach(response['output']) # 2024/05/28
        # 代わりに以下を追加
        userinterface.text_to_speach(response['output']) # 2024/05/28
        ...
        if self.isMtgStart:
            ...
            if args in Action_List:
                ...
                if ("WALKING" in args) or ("RUNNING" in args): # 次の会議場所に向かっている
                    ...
                    print("##### 予定と行動状態から提案する機能: #####")
                    print("*****\n次の予定ガイダンス\n*****") # 提案する機能
                    # model.text_to_speach("予定と行動状態から提案する機能は「次の予定確認」です。\n実行しますか？")
                    userinterface.text_to_speach("予定と行動状態から提案する機能は「次の予定確認」です。\n実行しますか？") # 2024/05/28
                    # ここから追加 (2024/05/27)
                    print("\n\n#####\nユーザーによる最終確認中...\n#####")
                    print("***** センシング中 *****")
                    # args = trigger.run()
                    ...
                    if Final_Check in "YES":
                        print("ユーザーの回答: ", Final_Check)
                        print(" > 実行します")
                        # model.text_to_speach("「予定管理」を実行します。")
                        userinterface.text_to_speach("「予定管理」を実行します。") # 2024/05/28
    ...
    def RestaurantForLunchApp(self, Action_List, trigger, agent, model):
        ...
        if ("Chewing" in args): # 咀嚼を検知
            ...
            print("##### 予定と行動状態から提案する機能: #####")
            # print("\n昼食の時間です。レストラン検索をします。") # 提案する機能
            print("*****\nレストラン検索\n*****")
            # ここから追加 (2024/05/27)
            # model.text_to_speach("予定と行動状態から提案する機能は「レストラン検索」です。\n実行しますか？")
            userinterface.text_to_speach("予定と行動状態から提案する機能は「レストラン検索」です。\n実行しますか？") # 2024/05/28
            print("\n\n#####\nユーザーによる最終確認中...\n#####")
            print("***** センシング中 *****")
            # args = trigger.run()
            # 音声入力の場合は入力の揺らぎをLLMによって最終出力をYES/NOにする
            ...
            if Final_Check in "YES":
                print("ユーザーの回答: ", Final_Check)
                print(" > 実行します")
                # model.text_to_speach("「レストラン検索」を実行します。")
                userinterface.text_to_speach("「レストラン検索」を実行します。") # 2024/05/28
    ...
    def GymTrainingApp(self, Action_List, trigger, agent, model):
        print("***** センシング中 *****")
        # args = trigger.run()
        args = "RUNNING" # テスト用
        print("DNN検出：", args)
        ...
        if args in Action_List:
            if ("WALKING" in args) or ("RUNNING" in args): # ジムで動いているなら楽曲再生
                ...
                if Final_Check in "YES":
                    ...
                    # 2024/05/27 追加
                    import subprocess
                    subprocess.run(['C:/Users/0107409377/.pyenv/pyenv-win/versions/3.12.0/python.exe', 'C:/Users/0107409377/Desktop/code/AtCoder/src/DemoApp/GymSpotify.py'])
                    # model.text_to_speach("STABLEが検出されたので停止します。")
                    userinterface.text_to_speach("STABLEが検出されたので停止します。") # 2024/05/28
