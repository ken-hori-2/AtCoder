...
# n_layers  = 1
# model = My_rnn_net(n_inputs, n_outputs, n_hidden, n_layers) # RNNをインスタンス化
...
for epoch in range(num_epochs):
    ...
    for inputs, targets in train_loader:
        ...
        # predictions = (outputs > 0.5).astype(int)  # 閾値0.5で分類
...
# predictions_prob = outputs # torch.sigmoid(outputs).detach().numpy()  # シグモイド関数で0-1の範囲にスケール
...
# # predictions = (outputs > 0.5).float().numpy()  # 閾値0.5で分類
# predictions = (outputs > threshold).float().numpy()  # 閾値0.8で分類
