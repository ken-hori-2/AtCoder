...
def generate_dummy_data(num_samples_per_class):
    ...
    return behavior_data, environment_data, time_data, labels
...
def preprocess_data(behavior_data, environment_data, time_data):
    # カテゴリカルデータを数値に変換
    ...
    data = np.hstack((behavior_data.reshape(-1, 1), environment_data.reshape(-1, 1), time_data.reshape(-1, 1)))
...
class MultiModalModel(nn.Module):
    def __init__(self):
        ...
        self.fc1 = nn.Linear(3, 128)  # 入力サイズは3（行動状態、周辺環境、時刻）
...
behavior_data, environment_data, time_data, labels = generate_dummy_data(num_samples_per_class)

# データの前処理
# data = preprocess_data(behavior_data, environment_data, location_data, time_data)
data = preprocess_data(behavior_data, environment_data, time_data)
...
for i in range(num): # num_samples_per_class):
    ...
    time = time_mapping[data[i, 2].item()]
